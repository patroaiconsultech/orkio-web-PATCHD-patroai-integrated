...
function TranscriptBlock({ title, text, emptyText }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ color: clean ? "rgba(255,255,255,0.92)" : "rgba(255,255,255,0.48)", lineHeight: 1.58, whiteSpace: "pre-wrap" }}>
        {clean || emptyText}
      </div>
    </div>
  );
...
export default function RealtimeTranscriptSummary({
  ...
  const copyText = useMemo(() => buildCopyText(summary || {}), [summary]);
  const userText = normalizeText(summary?.userText);
  const assistantText = normalizeText(summary?.assistantText);
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 18 }}>
          <div>
            ...
            <div style={{ fontSize: "clamp(22px, 4vw, 34px)", fontWeight: 950, letterSpacing: "-0.04em" }}>
              Resumo da conversa por voz
            </div>
            <div style={{ marginTop: 6, color: "rgba(255,255,255,0.58)", fontSize: 13 }}>
              {summary?.endedAt ? `Encerrada às ${formatTime(summary.endedAt)}` : "Sessão encerrada"}
            </div>
          </div>
        ...
        <div style={{ display: "grid", gap: 14 }}>
          ...
          <TranscriptBlock
            ...
            text={assistantText}
            emptyText="Nenhuma resposta final do Orkio foi registrada nesta sessão."
          />
        </div>
