...
export default function BetaAccessGate({ children = null }) {
  ...
  async function submitWaitlist(e) {
    ...
    if (!form.consent) {
      setError("Autorize o contato para entrar na Lista Prioritária.");
      return;
    }
  ...
  const shell = {
    minHeight: "100vh",
    background:
      "radial-gradient(circle at 20% 10%, rgba(124,92,255,0.26), transparent 32%), radial-gradient(circle at 82% 0%, rgba(245,158,11,0.18), transparent 30%), #070914",
    color: "#fff",
  ...
  return (
    <main style={shell}>
      <section style={card}>
        <div style={{ fontSize: 12, letterSpacing: "0.18em", textTransform: "uppercase", color: "#facc15", fontWeight: 900 }}>
          ORKIO OS
        </div>

        <h1 style={{ margin: "12px 0 10px", fontSize: "clamp(30px, 6vw, 52px)", lineHeight: 1.02 }}>
          Programa de Evolução Controlada
        </h1>

        {!codeAccepted ? (
          <>
            <p style={{ color: "rgba(255,255,255,0.74)", lineHeight: 1.65, fontSize: 16, maxWidth: 620 }}>
              Estamos realizando melhorias estruturais na nova geração do ORKIO OS.
              Neste momento, o acesso antecipado está temporariamente restrito para garantir
              a melhor experiência possível.
            </p>

            <form onSubmit={submitCode} style={{ marginTop: 24, display: "grid", gap: 12 }}>
              <label style={{ display: "grid", gap: 8 }}>
                <span style={{ color: "rgba(255,255,255,0.78)", fontWeight: 800 }}>Código de acesso</span>
                <input
                  value={code}
          ...
          <div style={{ marginTop: 24, display: "grid", gap: 14 }}>
            <h2 style={{ margin: 0, fontSize: 26 }}>Obrigado pelo interesse.</h2>
            <p style={{ color: "rgba(255,255,255,0.76)", lineHeight: 1.65 }}>
              Seu cadastro foi registrado com sucesso. Nossa equipe entrará em contato assim que
              a próxima fase do ORKIO estiver disponível para acesso.
            </p>
            <div style={{ color: "#facc15", fontWeight: 900 }}>Equipe ORKIO • PATROAI</div>
          </div>
        ) : (
          <>
            <p style={{ color: "rgba(255,255,255,0.74)", lineHeight: 1.65, fontSize: 16 }}>
              O código foi reconhecido. Para esta fase, vamos registrar seu interesse na Lista Prioritária.
            </p>

            <form onSubmit={submitWaitlist} style={{ marginTop: 22, display: "grid", gap: 12 }}>
              <input style={field} value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="Nome" />
              <input style={field} value={form.company} onChange={(e) => setForm((p) => ({ ...p, company: e.target.value }))} placeholder="Empresa" />
              <input style={field} value={form.whatsapp} onChange={(e) => setForm((p) => ({ ...p, whatsapp: e.target.value }))} placeholder="WhatsApp" />
              <input style={field} value={form.email} onChange={(e) => setForm((p) => ({ ...p, email: e.target.value }))} placeholder="E-mail" type="email" />

              <label style={{ display: "flex", gap: 10, alignItems: "flex-start", color: "rgba(255,255,255,0.76)", lineHeight: 1.45 }}>
                <input
                  type="checkbox"
                  ...
                  style={{ marginTop: 3 }}
                />
                <span>Autorizo a equipe ORKIO/PATROAI a entrar em contato sobre atualizações da plataforma.</span>
              </label>
